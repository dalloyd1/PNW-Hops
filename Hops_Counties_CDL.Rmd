...
[Attach elevation to hops CDL points](#add-elevation-data-to-hops-cdl-points)
...
It is worth noting for future reference that, regardless of the given resolution, longitude is reported to four digits and latitude to five digits. This analysis does not test for gaps in CDL coverage between POINTs in the geoemtry.
...
The CDL data remain in NAD83 / Conus Albers CRS, with integer UTM XY points as found in the original raster files. 

```{r test_distance_errors, eval=FALSE, include=FALSE}
######################################################
##
##  distance errors from lon/lat coordinate rounding
##
cdl_sample <- 
  cdl_hops_points_unique %>%
  filter(fips == 41047) %>%
...
tic()
tmp_de <-
  lapply(6:1, function(digits) {
    cdl_sample_no_geom <-
      cdl_sample %>%
      st_drop_geometry() %>%
      mutate(lon = round(lon, digits)
             ,lat = round(lat, digits)
      ) 
    pts_remaining <-
      c(digits
        ,nrow(cdl_sample)
        ,nrow(distinct(dplyr::select(cdl_sample_no_geom, -xres, -yres)))
      )
    dist_err <- st_distance(cdl_sample, st_as_sf(cdl_sample_no_geom, coords = c("lon", "lat"), crs = 4269), by_element = T)
    mutate(cdl_sample_no_geom, digits, dist_err)
  }) %>%
  bind_rows
toc()
...
tmp_de %>%
  ...
  ggplot(aes(x=dist_err, group=digits, fill=factor(digits))) +
  geom_histogram(show.legend = F) +
  facet_wrap(~ digits, scales="free")
```
...
st_write(cdl_hops_points_distinct, 
         here(SUBDIR, "cdl_hops_points_distinct.gpkg"), 
...
First, let's do some timing and accuracy checks using a small sample of hops CDL points from Marion County, OR.
...
Fall 2025 -- added logic to avoid downloading TIFs unnecessarily by testing for saved `cdl_hops_points_by_year.gpkg` as a starting point for `tif_req` 

May 2026 -- added call to AWS Terrain Tiles via elevatr before write of GPKG output
