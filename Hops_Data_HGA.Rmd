...
```{r precombined_data_tests, eval=FALSE, include=FALSE}
# yr_hga <- pull(hga_yield, Year) %>% unique %>% sort
# range(yr_hga, finite = T)
# # any missing values on HGA year range?
# any(min(yr_hga):max(yr_hga) -yr_hga >0)
# 
# yr_nass <- pull(nass_yield, Year) %>% unique %>% sort
# range(yr_nass, finite = T)
# # any missing values on NASS year range?
# any(min(yr_nass):max(yr_nass) -yr_nass >0)
# 
```
...
```{r HGA_Validation_2_Errors, eval=FALSE, include=FALSE}
hga_errors <- 
  left_join(
    select(hga_yield, Variety, Year, State, Yield, Source) %>%
      mutate(Match = toupper(Variety))
    ,select(nass_yield, Variety, Year, State, Yield)
...
```{r HGA_Discrepancies, eval=FALSE, include=FALSE}
filter(state_errors, frac_err >0)

filter(hga_errors, frac_err >0)
```

```{r missing_from_hga, eval=FALSE, include=FALSE}
...
```{r HGA_Validation_3_Correct_Yields, eval=FALSE, include=FALSE}
...
```{r Prelim_Analysis_pnw, eval=FALSE, include=FALSE}
hops_us_yields %>%
...
```{r pnw_outliers, eval=FALSE, include=FALSE}
...
```{r prelim_analysis_states, eval=FALSE, include=FALSE}
state_yields <- mutate(hops_state_yields, obs = row_number())
...
outliers <- filter(state_outlier_check, outlier)
...
```{r prelim_analysis_varieties, eval=FALSE, include=FALSE}
...
n_yr <- with(hops_var_yields, max(Year) -min(Year) +1)
...
tmp <- filter(state_yields_ready, Outlier) %>%
  ...
  left_join(hops_var_yields, by = c("State", "Year"), suffix = c(".pnw", "")) %>%
  filter(!is.na(Yield)) %>%
...
```{r trend_over_time, eval=FALSE, include=FALSE}
```
...
1. _There is a practical upper limit to the yield that is unknown from these data._ There is a limit to how many plants can be cultivated per acre from the space each plant needs to grow, and possibly the preferred spacing between plants to facilitate harvest. The number of cones produced per bine may vary naturally by hop variety.
